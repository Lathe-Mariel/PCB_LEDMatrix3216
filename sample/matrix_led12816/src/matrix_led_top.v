module matrix_led_top (
    ...
    localparam integer DISPLAY_COLS     = 128;
    ...
    localparam integer ROW0_IS_TOP       = 0;
    ...
    reg [3:0]  scan_row;
    reg [6:0]  bit_index;
    reg [63:0] row_shift_word;
    reg [63:0] col_shift_word_sin2;
    reg [63:0] col_shift_word_sin3;
    reg [127:0] display_row_bits [0:15];
    reg [13:0] hold_count;
    reg [22:0] scroll_tick_count;
    reg [8:0]  scroll_offset;
    integer row_iter;

    function [63:0] row_word_for;
        input [3:0] row_idx;
        integer panel_row;
        ...
        begin
            ...
            row_word_for = {one_hot_row, one_hot_row, one_hot_row, one_hot_row};
        end
    endfunction

    function serial_pick64;
        input [63:0] word;
        input [6:0] idx;
        begin
            if (SHIFT_MSB_FIRST != 0) begin
                serial_pick64 = word[idx];
            end else begin
                serial_pick64 = word[63 - idx];
            end
        end
    endfunction
    ...
    function message_pixel_at;
        input integer text_x;
    ...
    function [63:0] column_word_sin2_for;
        input [127:0] row_bits;
        integer col;
        integer panel_col;
        integer lane_bit;
        begin
            column_word_sin2_for = 64'd0;
            for (col = 0; col < DISPLAY_COLS; col = col + 1) begin
                ...
                if (row_bits[col]) begin
                    ...
                    end else if ((panel_col >= 48) && (panel_col < 64)) begin
                        lane_bit = panel_col - 32;
                        column_word_sin2_for[lane_bit] = 1'b1;
                    end else if ((panel_col >= 80) && (panel_col < 96)) begin
                        lane_bit = panel_col - 48;
                        column_word_sin2_for[lane_bit] = 1'b1;
                    end else if ((panel_col >= 112) && (panel_col < 128)) begin
                        lane_bit = panel_col - 64;
                        column_word_sin2_for[lane_bit] = 1'b1;
                    end
                end
            end
    ...
    function [63:0] column_word_sin3_for;
        input [127:0] row_bits;
        integer col;
        integer panel_col;
        integer lane_bit;
        begin
            column_word_sin3_for = 64'd0;
            for (col = 0; col < DISPLAY_COLS; col = col + 1) begin
                panel_col = COL0_IS_LEFT ? col : (DISPLAY_COLS - 1 - col);
                if (row_bits[col]) begin
                    if (panel_col < 16) begin
                        ...
                        lane_bit = panel_col - 16;
                        column_word_sin3_for[lane_bit] = 1'b1;
                    end else if ((panel_col >= 64) && (panel_col < 80)) begin
                        lane_bit = panel_col - 32;
                        column_word_sin3_for[lane_bit] = 1'b1;
                    end else if ((panel_col >= 96) && (panel_col < 112)) begin
                        lane_bit = panel_col - 48;
                        column_word_sin3_for[lane_bit] = 1'b1;
                    end
                end
    ...
    always @(posedge clk or negedge USER_KEY) begin
        if (!USER_KEY) begin
            ...
            scan_row          <= 4'd0;
            bit_index         <= 7'd63;
            row_shift_word    <= 64'd0;
            col_shift_word_sin2 <= 64'd0;
            col_shift_word_sin3 <= 64'd0;
            hold_count        <= 14'd0;
            scroll_tick_count <= 23'd0;
            scroll_offset     <= 9'd0;
            for (row_iter = 0; row_iter < DISPLAY_ROWS; row_iter = row_iter + 1) begin
                display_row_bits[row_iter] <= 128'd0;
            end
            SIN1              <= 1'b0;
            SIN2              <= 1'b0;
            SIN3              <= 1'b0;
            ...
            if (scroll_tick_count == SCROLL_TICK_CYC - 1) begin
                scroll_tick_count <= 23'd0;
                for (row_iter = 0; row_iter < DISPLAY_ROWS; row_iter = row_iter + 1) begin
                    display_row_bits[row_iter] <= {
                        display_row_bits[row_iter][DISPLAY_COLS - 2:0],
                        message_pixel_at(scroll_offset, row_iter)
                    };
                end
            ...
            case (state)
                STATE_LOAD: begin
                    ...
                    col_shift_word_sin2 <= column_word_sin2_for(display_row_bits[scan_row]);
                    col_shift_word_sin3 <= column_word_sin3_for(display_row_bits[scan_row]);
                    bit_index      <= 7'd63;
                ...
                STATE_SHIFT_SETUP: begin
                    LED_CLK <= 1'b0;
                    SIN1    <= serial_pick64(row_shift_word, bit_index);
                    SIN2    <= serial_pick64(col_shift_word_sin2, bit_index);
                    SIN3    <= serial_pick64(col_shift_word_sin3, bit_index);
